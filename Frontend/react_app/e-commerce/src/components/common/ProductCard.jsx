...
function ProductCard({ product, variant = 'default' }) {
  ...
  const {
    id,
    name = product?.description || 'Sneakers',
    price = 84.00,
    originalPrice,
    rating = 4.7,
    reviewCount = 89,
  ...
  const displayImage = image_url
    ? (image_url.startsWith('http') ? image_url : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}${image_url}`)
    : image;
  ...
  useEffect(() => {
    const checkWishlist = async () => {
      if (!id) return;
      try {
        const result = await api.checkWishlistStatus(id);
        setIsWishlisted(result.in_wishlist);
      } catch (error) {
        // Silent error for wishlist check
      }
    };
    checkWishlist();
  }, [id]);
